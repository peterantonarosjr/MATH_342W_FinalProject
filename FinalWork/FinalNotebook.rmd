...
Ximp = missForest(imputeSet,verbose = TRUE, ntree = 500, parallelize = "forests")
...
#Let's leave ~10% of our total data as a final honest holdout
holdout_K=10
holdout_prop = 1 / holdout_K

#Where sale price was NA prior to imputing we leave for training ~ 75%
salePriceNA_Data = finalHousingData[salePriceMissingIndices,] 

#This is crucial to note since our errors will be more honest. 
...
#Where sale price was not NA we leave for testing ~ 25%
salePriceFilled_Data = finalHousingData[salePriceFilledIndices,]

#Training & Testing data (All Features)
#finalHousingData_trainIndices = sample(1 : nrow(finalHousingData), round((1 - holdout_prop) * nrow(finalHousingData)))
#finalHousingData_testIndices =  setdiff(1 : nrow(finalHousingData), finalHousingData_trainIndices)

#finalHousingData_Train = finalHousingData[finalHousingData_trainIndices,] 
#finalHousingData_Test = finalHousingData[finalHousingData_testIndices,] #This is our holdout set here
...
control_selection =  rfeControl(functions=rfFuncs, method="repeatedcv", number=5,repeats=3)
...
#Here we see that there are 25 total columns. We expect at most 25 perfect collinearities in our pxp correlation matrix when i==j
#Greater than 25 columns indicates that there is perfect collinearity when i!=j

correlationMatrix2 = as.matrix(cor(numericOnlyData2))

length(which(correlationMatrix2==1)) # 27 matches for perfect correlation there is an issue with this


cor(finalHousingData_Train[,"sale_price_miss"],finalHousingData_Train[,"date_of_sale_miss"]) # These are the 2 perfectly correlated columns where i!=j

#This perfect correlation existing makes conceptual sense, since a sale price missing implies there is no date of sale.
```
...
varImp(trained_selection) #Both date_of_sale_miss and sale_price_miss score low and nearly identically
...
subsetF = subsetF[!subsetF %in% c("date_of_sale_miss")]
...
```{r}
#Let's leave ~10% of our total data for testing
K=10
prop = 1 /K
